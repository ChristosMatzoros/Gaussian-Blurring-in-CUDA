...
* tiling with multiple kernel invocations. We use streams for better performance.
* 
*/
...
#define IMAGE_DIM (16384)	//width size of the image
...
__global__ void convolutionGPUColumn(double *d_Output, double *d_Buffer, int imageW, int imageH, int filterR) {
	...
	double result;														//Result variable keeps the result during the convolution for each pixel
...
int main(int argc, char **argv) {
	...
	cudaStream_t stream0,stream1;		//Pointers to new stream identifiers
	cudaStreamCreate(&stream0);			//Create an asynchronous stream(stream0)
	cudaCheckError()
	cudaStreamCreate(&stream1);			//Create an asynchronous stream(stream1)
	cudaCheckError()
	...
	//host memory used for transfers between host and device after each invocation 
	...
	Check_Allocation_Return_Value(hostparBuff0)
	hostparBuff1 = (double *)malloc(BLOCKED_TILE * BLOCKED_TILE * sizeof(double));
	Check_Allocation_Return_Value(hostparBuff1)
	...
	//Initialization of image and filter
	srand(200);
	...
	for(k=0;k<N;k++){
		for(l=0;l<N;l+=2){
			//take a part(tile) of the whole image for each kernel invocation
			...
			cudaMemcpyAsync(d_Input0, host_Grid_Input0, BLOCKED_TILE * PADDED_WIDTH * sizeof(double),cudaMemcpyHostToDevice,stream0);
			cudaCheckError()
			//Copy host array to device1
			cudaMemcpyAsync(d_Input1, host_Grid_Input1, BLOCKED_TILE * PADDED_WIDTH * sizeof(double),cudaMemcpyHostToDevice,stream1);
			cudaCheckError()
			
			//convolutionGPURow First invocation
			convolutionGPURow<<<dimGrid,dimBlock,0,stream0>>>(d_Buffer0, d_Input0, BLOCKED_TILE, BLOCKED_TILE, FILTER_RAD);
			cudaCheckError()
			//convolutionGPURow Second invocation
			convolutionGPURow<<<dimGrid,dimBlock,0,stream1>>>(d_Buffer1, d_Input1, BLOCKED_TILE, BLOCKED_TILE, FILTER_RAD);
			cudaCheckError()
			
			//Buffer0 copy device to host
			cudaMemcpyAsync(hostparBuff0, d_Buffer0, BLOCKED_TILE* BLOCKED_TILE * sizeof(double), cudaMemcpyDeviceToHost,stream0);
			cudaCheckError()
			//Buffer1 copy device to host
			cudaMemcpyAsync(hostparBuff1, d_Buffer1, BLOCKED_TILE* BLOCKED_TILE * sizeof(double), cudaMemcpyDeviceToHost,stream1);	//cudaCheckError()
	...
	for(k=0;k<N;k++){
		for(l=0;l<N;l+=2){
			...
			cudaMemcpyAsync(d_Buffer0, host_Grid_Input0, BLOCKED_TILE * PADDED_WIDTH * sizeof(double),cudaMemcpyHostToDevice,stream0);
			cudaCheckError()
			//Copy host array to device(stream1)
			cudaMemcpyAsync(d_Buffer1, host_Grid_Input1, BLOCKED_TILE * PADDED_WIDTH * sizeof(double),cudaMemcpyHostToDevice,stream1);
			cudaCheckError()
			
			//convolutionGPUColumn Stream0
			convolutionGPUColumn<<<dimGrid,dimBlock,0,stream0>>>(d_Output0, d_Buffer0,BLOCKED_TILE, BLOCKED_TILE, FILTER_RAD);
			cudaCheckError()
			//convolutionGPUColumn Stream1
			convolutionGPUColumn<<<dimGrid,dimBlock,0,stream1>>>(d_Output1, d_Buffer1,BLOCKED_TILE, BLOCKED_TILE, FILTER_RAD);
			cudaCheckError()
			
			//Buffer0 copy device to host
			cudaMemcpyAsync(hostparBuff0, d_Output0, BLOCKED_TILE* BLOCKED_TILE * sizeof(double), cudaMemcpyDeviceToHost,stream0);
			cudaCheckError()
			//Buffer1 copy device to host
			cudaMemcpyAsync(hostparBuff1, d_Output1, BLOCKED_TILE* BLOCKED_TILE * sizeof(double), cudaMemcpyDeviceToHost,stream1);	cudaCheckError()
			cudaDeviceSynchronize();
	...
	//Destroys and cleans up the asynchronous stream specified by "stream0"
	cudaStreamDestroy(stream0);
	//Destroys and cleans up the asynchronous stream specified by "stream1"
